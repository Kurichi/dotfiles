...
```bash
# 自動判定で適用（hostname からプロファイルを自動選択）
nix run .#switch

# 手動指定
sudo darwin-rebuild switch --flake .#personal  # or .#work

# テストビルド（適用せずにビルドのみ、hostname で自動判定）
nix run .#build
...
nix/
├── hosts/                    # プロファイルデータ（personal/work）
│   ├── personal.nix          # personal マシン設定（username, hostname, git, homebrew, packages）
│   └── work.nix              # work マシン設定
├── modules/
│   ├── darwin/               # macOS システム設定
