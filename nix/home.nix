...

{
  ...
  # Git configuration (new syntax)
  programs.git = {
    enable = true;
    settings = {
      user.name = "Kurichi";
      user.email = "yuuyapp080@gmail.com";
  ...
  programs.fish = {
    ...
    shellAbbrs = {
      ...
      k8s = "kubectl";
      # nix
      rebuild = "darwin-rebuild switch --flake ~/repos/dotfiles#Kurichi-MacBook-Pro";
    };
    functions = {
      fish_prompt = ''
        # Cache exit status
  ...
  # Starship prompt (disabled - using custom fish_prompt)
  programs.starship = {
    enable = false;
  };
