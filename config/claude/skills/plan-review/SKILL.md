...
name: plan-review
description: |
  Proactively initiates Codex CLI review immediately after the user approves a plan (before the first Edit/Write).
  Use this skill AUTOMATICALLY right after ExitPlanMode is approved, before making any changes, to get peer review on implementation plans (trivial changes may skip).
...
Automatically leverage Codex CLI (`codex exec`) for plan review right after the user approves a plan. This skill uses SubAgent (Task tool) to isolate review output from main context, preventing context window bloat.

## When to Use

**IMPORTANT: This skill should be invoked AUTOMATICALLY immediately after the user approves the plan (ExitPlanMode returns approval), before the first Edit/Write（軽微な変更を除く）。**

Plan Mode 中は SubAgent も書き込み・実行系ツール（Bash 実行等）を使えないため、`codex exec` を伴うこのレビューは ExitPlanMode より前には実行できない。そのため「プラン承認直後・最初の実装アクションより前」を必須のチェックポイントとする。問題が見つかった場合は、それ以上実装を進めずに Plan Mode に戻ってプランを修正すること。

Use this skill in these scenarios:

1. **After plan approval, before first edit** - Automatically when the user has approved a non-trivial implementation plan and before touching any files
...
```
[ExitPlanMode called -> user approves plan]
         |
[Invoke plan-review skill (before any Edit/Write)]
         |
...
[Assess change complexity from plan content]
   ...
   +- Skip condition     -> [Skip + 明記] -> [Proceed to implementation]
   +- Otherwise/Unsure   -> [Launch SubAgent: Standard tier]
                              ...
                              [Main context receives summary only]
                                    |
                                +- Approved -> Proceed to implementation (first Edit/Write)
                                +- Issues found -> Return to Plan Mode, modify plan -> Re-launch SubAgent -> Loop
...
umask 077
PLAN_FILE="{プランファイルパス}"
REVIEW_OUTPUT=$(mktemp "${TMPDIR:-/tmp}/codex-review-XXXXXX")
REVIEW_ERR=$(mktemp "${TMPDIR:-/tmp}/codex-review-err-XXXXXX")
trap 'rm -f "$REVIEW_OUTPUT" "$REVIEW_ERR"' EXIT

# ハードタイムアウト（外部の timeout/gtimeout バイナリに依存しない自前実装）
MAX_WAIT=600   # Deep tier: 600秒（Standard tier は 300秒）
POLL_INTERVAL=5
...
while kill -0 "$CODEX_PID" 2>/dev/null; do
  ...
  if [ "$ELAPSED" -ge "$MAX_WAIT" ]; then
    ...
    echo "ERROR: codex exec exceeded ${MAX_WAIT}s, killed" >&2
    exit 1
  fi
done
...
注意: `$TMPDIR` はサンドボックスが書き込みを許可する一時ディレクトリ。`/tmp` に直接書き込むと sandbox に拒否される。
注意: バックグラウンド実行 + PID ポーリングで `MAX_WAIT` 秒を超えたら強制終了する。`codex exec` が子プロセスを再起動するケースでは孫プロセスが残る可能性があるが、スクリプト自体は確実に `MAX_WAIT` 秒以内に復帰する。
...
Deep tier と同じスクリプト構造だが、以下が異なる：

```bash
# 変更箇所のみ：
# MAX_WAIT=600 → MAX_WAIT=300
...
1. **スクリプト内蔵のハードタイムアウト（主）**: 上記テンプレートの `MAX_WAIT` によるバックグラウンド実行 + PID ポーリング + 強制 kill。Deep tier: 600秒、Standard tier: 300秒。外部の `timeout`/`gtimeout` バイナリに依存しない。
2. **Bash ツール呼び出し時の timeout（保険）**: スクリプト内のループ以外の箇所（`cat` 等）で万一止まった場合に備え、Bash ツール呼び出し時にも timeout を指定する：
   - Deep tier: `600000` (10分、内蔵タイムアウトに数十秒のマージンを加算)
   - Standard tier: `300000` (5分、同上)
...
3回反復しても通過しない場合：
1. 残りの懸念事項をユーザーに要約
2. ユーザーに判断を仰ぐ（実装を続行 / Plan Mode に戻ってプラン修正 / 中止）
...
プラン承認後・最初の Edit/Write より前に本スキルを実行する。ExitPlanMode 自体は既に承認済みのため、ここでの分岐は「実装に進むか、Plan Mode に戻ってプランを修正するか」。

### Reviewed by Codex（SubAgent 経由）
1. SubAgent から返された要約をユーザーに提示
2. 「Codex reviewed and approved (via SubAgent)」と明記
3. 実装（最初の Edit/Write）に進む
